...
describe("Audit Regressions", () => {
  ...
  it("credits fee amounts correctly when the fee recipient is the payee in individual settlement", async () => {
    ...
    await program.methods
      .deposit(1, new anchor.BN(5_000_000))
      .accounts({
    ...
    const message = createCommitmentMessage({
      ...
      feeRecipientId: channel.payeeId,
    });
  ...
  it("preserves net payer balances when the fee recipient is the payer in individual settlement", async () => {
    const payer = await createTestParticipant();
    const payee = await createTestParticipant();
    const payerTokenAccount = await createFundedTokenAccount(
      payer.wallet,
    ...
    const message = createCommitmentMessage({
      ...
      tokenId: 1,
      feeAmount: new anchor.BN(feeAmount),
      feeRecipientId: channel.payerId,
    });
  ...
  it("does not lose fee credits when the fee recipient is one of the bundle payers", async () => {
    const payee = await createTestParticipant();
    const payerA = await createTestParticipant();
    const payerB = await createTestParticipant();
    const payerATokenAccount = await createFundedTokenAccount(
      payerA.wallet,
      primaryMint,
      10_000_000
    );
    const payerBTokenAccount = await createFundedTokenAccount(
      payerB.wallet,
      primaryMint,
      10_000_000
    );
    ...
    await program.methods
      .deposit(1, new anchor.BN(5_000_000))
      .accounts({
        owner: payerA.wallet.publicKey,
        participantAccount: payerA.participantPda,
        ownerTokenAccount: payerATokenAccount,
        vaultTokenAccount: findVaultTokenAccountPda(1),
      } as any)
      .signers([payerA.wallet])
      .rpc();
    await program.methods
      .deposit(1, new anchor.BN(5_000_000))
      .accounts({
        owner: payerB.wallet.publicKey,
        participantAccount: payerB.participantPda,
        ownerTokenAccount: payerBTokenAccount,
        vaultTokenAccount: findVaultTokenAccountPda(1),
      } as any)
      .signers([payerB.wallet])
      .rpc();

    const channelA = await ensureChannel(payerA.wallet, payee.wallet.publicKey, 1);
    const channelB = await ensureChannel(payerB.wallet, payee.wallet.publicKey, 1);

    const payerABefore = await program.account.participantAccount.fetch(
      payerA.participantPda
    );
    const payerBBefore = await program.account.participantAccount.fetch(
      payerB.participantPda
    );
    const payeeBefore = await program.account.participantAccount.fetch(
    ...
    const bundleEntries = [
      {
        ...
        message: createCommitmentMessage({
          ...
          feeRecipientId: channelA.channel.payerId,
        }),
      },
      {
        signer: payerB.wallet,
        message: createCommitmentMessage({
    ...
    await program.methods
      ...
      .remainingAccounts([
        { pubkey: payerA.participantPda, isWritable: true, isSigner: false },
        { pubkey: channelA.channelPda, isWritable: true, isSigner: false },
        { pubkey: payerB.participantPda, isWritable: true, isSigner: false },
        { pubkey: channelB.channelPda, isWritable: true, isSigner: false },
      ])
      .preInstructions([bundledEd25519Ix])
      .signers([payee.wallet])
      .rpc();

    const payerAAfter = await program.account.participantAccount.fetch(
      payerA.participantPda
    );
    const payerBAfter = await program.account.participantAccount.fetch(
      payerB.participantPda
    );
    const payeeAfter = await program.account.participantAccount.fetch(
      payee.participantPda
    ...
    expect(
      getTokenBalance(payerAAfter, 1).availableBalance.toNumber() -
        getTokenBalance(payerABefore, 1).availableBalance.toNumber()
    ).to.equal(-1_000_000);
    expect(
      getTokenBalance(payerBAfter, 1).availableBalance.toNumber() -
        getTokenBalance(payerBBefore, 1).availableBalance.toNumber()
    ).to.equal(-1_500_000);
    expect(
      getTokenBalance(payeeAfter, 1).availableBalance.toNumber() -
        getTokenBalance(payeeBefore, 1).availableBalance.toNumber()
